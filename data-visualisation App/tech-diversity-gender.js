...
function TechDiversityGender() {
  ...
  this.draw = function () {
    ...
    // It draws Female/Male labels at the top of the plot.
    this.drawCategoryLabels();

    var lineHeight = (height - this.layout.topMargin) /
      this.data.getRowCount();

    // Loops over every row in the data.
    for (var i = 0; i < this.data.getRowCount(); i++) {

      // Calculates the y position for each company.
      var lineY = (lineHeight * i) + this.layout.topMargin;

      // Creates an object that stores data from the current row.
      var company = {
      ...
      // It draws the company name in the left margin.
      fill(0);
      noStroke();
      textAlign('right', 'top');
      text(company.name,
      ...
      // It draws female employees rectangle.
      ...
      // It drwas the male employees rectangle.
